...
**v1.1 顺序：M10 → M11 → M12 → M13 → M14 → M15**。M10 与 M11 都改 locations.js，**不要并行**；M12 必须在 M10/M11 之后（要复核它们的新文本）；M15 依赖 M14 的 warmth 属性，必须最后。M13–M15 完成后建议加跑一轮 M12 式文本自查。
每个模块开一个新窗口，用 `/model` 切到进度表标注的模型再开工。

## 交接备注（每模块完成后追加，最新在上）

### v1.1 拍板（2026-07-17, fable）—— 仍只是计划，未动游戏代码
